...
def make_arg_parser():
    app_name="block-by-snp.py"
    description="generate x snp block definition file"
    ...
    parser.add_argument("--noheader",
            action="store_true",
            default=False,
            help="set if there is no header in the input files")
...
if __name__ == '__main__':
    ...
    for cf in file_list:
        ...
        f.close()
        if len(hap)<=2:
            num_hap = int(len(hap[-1])/2)
        else:
            num_hap = int((len(hap)-1)/2)
        rem=int(num_hap%sd)
        blks=int((num_hap-rem)/sd)
        if args.verbose:
        ...
        if (rem != 0):
            ...
            count=count+1
    if args.verbose: print(count-1, 'blocks total')
